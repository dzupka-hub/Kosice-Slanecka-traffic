...
// Hranice useku - overene kalibraciou 17.8.2026 (3,92 km v oboch smeroch)
const KRASNA = { lat: 48.668514, lon: 21.319647 };
const MESTO  = { lat: 48.689904, lon: 21.280907 };

// Bod pre bodove meranie rychlosti (stred Slaneckej)
...
async function meraj_trasu(zaciatok, koniec) {
  const url =
    `https://api.tomtom.com/routing/1/calculateRoute/${bod(zaciatok)}:${bod(koniec)}/json` +
...
(async () => {
  ...
  const HLAVICKA = [
    ...
    "do_mesta_cas_min", "do_mesta_volny_cas_min", "do_mesta_obvykly_cas_min", "do_mesta_dlzka_km",
    "do_krasnej_rychlost", "do_krasnej_volna", "do_krasnej_stav",
    "do_krasnej_cas_min", "do_krasnej_volny_cas_min", "do_krasnej_obvykly_cas_min", "do_krasnej_dlzka_km",
  ].join(";");
  ...
  function stlpce(m) {
    ...
    if (!t || t.error) out.push("", "", "", "");
      ...
      cislo(t.casObvykly / 60), cislo(t.dlzka / 1000, 2)
    );
    return out;
  }
